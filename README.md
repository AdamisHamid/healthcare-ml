...
**Logistic regression (planned)** will extend the same approach to binary classification, passing the linear combination through the sigmoid function $\sigma(z) = \frac{1}{1+e^{-z}}$ and using cross-entropy loss instead of MSE, since MSE doesn't optimise well for classification.
...
## v0 Results (1000 epochs, learning rate 0.01)
...
- Logistic regression from scratch, with cross-entropy loss
